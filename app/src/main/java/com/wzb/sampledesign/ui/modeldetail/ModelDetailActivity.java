...
public class ModelDetailActivity extends AppCompatActivity implements View.OnClickListener {
	...
	private void init(){
		Log.e("当前模型：",Constant.PROJECT_NAME);
		// 根据比例，在两个color值之间计算出一个color值
		final int fromColor = ContextCompat.getColor(this,R.color.colorHelper_square_from_ratio_background);
		final int toColor = ContextCompat.getColor(this,R.color.colorHelper_square_to_ratio_background);
		...
		mRatioSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){
			...
			public void onStartTrackingTouch(SeekBar seekBar) {
				// 刚刚触碰进度条
				showToast("当前的进度：" + seekBar.getProgress());
